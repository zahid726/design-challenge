...
function getPageFromPath(pagePath, pageExtensions) {
    ...
    const page = (0, _denormalizePagePath).normalizePathSep(pagePath.replace(new RegExp(`\\.+(${extensions.join('|')})$`), '')).replace(/\/index$/, '');
    return page === '' ? '/' : page;
}
function createPagesMapping({ hasServerComponents , isDev , pageExtensions , pagePaths  }) {
    ...
    const pages = pagePaths.reduce((result, pagePath)=>{
        ...
        result[pageKey] = (0, _denormalizePagePath).normalizePathSep((0, _path).join(_constants.PAGES_DIR_ALIAS, pagePath));
        return result;
    }, {});
...
function getEdgeServerEntry(opts) {
    ...
    const loaderParams = {
        ...
        dev: opts.isDev,
        isServerComponent: (0, _utils1).isFlightPage(opts.config, opts.absolutePagePath),
        page: opts.page,
        stringifiedConfig: JSON.stringify(opts.config)
    };
    _middlewarePlugin.ssrEntries.set(opts.bundlePath, {
        requireFlightManifest: (0, _utils1).isFlightPage(opts.config, opts.absolutePagePath)
    });
        ...
        runDependingOnPageType({
            ...
            onEdgeServer: ()=>{
                edgeServer[serverBundlePath] = getEdgeServerEntry({
                    ...
                    isDev: false,
                    page,
                    ssrEntries: _middlewarePlugin.ssrEntries
                });
            }
        });
    }));
