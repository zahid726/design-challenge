...
function getEntrypointInfo(compilation, { envPerRoute , wasmPerRoute  }) {
    ...
    for (const entrypoint of entrypoints.values()){
        ...
        ].filter(_nonNullable.nonNullable) : entryFiles.map((file)=>{
            return 'server/' + file;
        });
        infos.push({
...
class MiddlewarePlugin {
    ...
    createAssets(compilation, assets, { envPerRoute , wasmPerRoute  }) {
        ...
        middlewareManifest.clientInfo = middlewareManifest.sortedMiddleware.map((key)=>{
            ...
            return [
                key,
                !!ssrEntries.get(middleware.name)
            ];
        });
    ...
    compiler.hooks.compilation.tap(options.pluginName, (compilation, { normalModuleFactory  })=>{
        ...
        compilation.hooks.afterOptimizeModules.tap(PLUGIN_NAME, ()=>{
            ...
            for (const [name, info] of compilation.entries){
                if (info.options.runtime === _constants.EDGE_RUNTIME_WEBPACK) {
