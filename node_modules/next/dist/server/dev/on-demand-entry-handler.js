...
function onDemandEntryHandler(watcher, multiCompiler, { pagesDir , nextConfig , maxInactiveAge , pagesBufferLength  }) {
    ...
    return {
        async ensurePage (_page, clientOnly) {
            ...
            const promises = (0, _entries).runDependingOnPageType({
                page,
                pageRuntime: await (0, _entries).getPageRuntime(absolutePagePath, nextConfig),
            ...
            if (entriesChanged) {
                (0, _output).reportTrigger(!clientOnly && promises.length > 1 ? `${page} (client and server)` : page);
...
async function getPageInfo(opts) {
    ...
    if (pagePath.startsWith('next/dist/pages/')) {
        return {
            page: (0, _normalizePagePath).normalizePathSep(page),
            bundlePath: page,
    ...
    return {
        ...
        page: (0, _normalizePagePath).normalizePathSep(_path.posix.normalize(pageUrl))
    };
}
